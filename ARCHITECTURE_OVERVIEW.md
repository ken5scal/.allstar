...
6. AI は **外部 API** を利用する。  
7. 実行基盤は **ローカル実行 (Go)** で開始する。  
8. ローカルスケジューラは **macOS の標準機構 (launchd)** を想定する。  
9. スケジュールは設定ファイルで複数定義できる構成にする。  
10. 失敗時は **都度 Slack へエラー通知** する。  
11. 通知は定期ジョブではなく、エラーハンドリングモジュールとして扱う。  
...
2. Runner が設定を読み、対象ソースの収集を実行する。  
3. 取得データを Vault に 1 レコード単位で保存する。  
4. 新規/更新レコードに対して AI 要約を追記する。  
