...
describe("summarize selection e2e", () => {
  let stdoutSpy:
    | {
        mockRestore(): void;
        mock: { calls: Array<[string | Uint8Array<ArrayBufferLike>, ...unknown[]]> };
      }
    | undefined;
  ...
  it("caps summarize work per manual run and reports pending counts", async () => {
    const tmp = mkdtempSync(path.join(os.tmpdir(), "obsflow-summarize-e2e-"));
    ...
    for (const [name, capturedAt] of [
      ["older", "2026-05-05T10:00:00.000Z"],
      ["middle", "2026-05-05T11:00:00.000Z"],
      ["newer", "2026-05-05T12:00:00.000Z"],
    ] as const) {
    ...
    const older = await vault.readRecord("src/rss/sample/2026/05/05/older.md");
    const middle = await vault.readRecord("src/rss/sample/2026/05/05/middle.md");
    const newer = await vault.readRecord("src/rss/sample/2026/05/05/newer.md");
    expect(older?.status).toBe("captured");
    expect(middle?.status).toBe("captured");
    expect(newer?.status).toBe("summarized");
    ...
    if (!stdoutSpy) throw new Error("stdout spy missing");
    const stdout = stdoutSpy.mock.calls.map((call) => String(call[0])).join("");
    expect(stdout).toContain(
      "summarize-main: success processed=1 skipped=0 failed=0 pending=3 selected=1 deferred=2",
    );
  });
});
