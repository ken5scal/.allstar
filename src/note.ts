...
export function parseVaultNote(markdown: string): VaultRecord | null {
  ...
  return {
    record_id: String(fm.record_id ?? ""),
    schema_version: Number(fm.schema_version ?? 1),
    source: fm.source as VaultRecord["source"],
    source_item_key: String(fm.source_item_key ?? ""),
    content_hash: String(fm.content_hash ?? ""),
    status: fm.status as VaultRecord["status"],
    ai_drafted: Boolean(fm.ai_drafted),
    category: fm.category !== undefined ? String(fm.category) : undefined,
    ...
    tick_run_id: String(fm.tick_run_id ?? ""),
    job_run_id: String(fm.job_run_id ?? ""),
