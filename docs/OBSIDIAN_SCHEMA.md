...
| content status | `content_status` | string (enum, single value) | Text | no | `suspected_missing` | 人間が付与する本文品質フラグ。未設定は未フラグ |
| content issue note | `content_issue_note` | string | Text | no | `本文が冒頭だけで途切れている` | 本文欠落・薄さに関する補足メモ |
| content issue marked at | `content_issue_marked_at` | datetime (ISO8601) | Date & time | no | `2026-05-06T13:00:00Z` | 人間が本文品質フラグを付与した時刻 |
...
### 2.4 `content_status` enum (single-select)

`content_status` は人間が本文品質を後からマークするための任意プロパティである。未設定は「未フラグ」として扱う。

- `ok` - 本文に問題なし、または手動補正済み
- `suspected_missing` - 本文欠落・薄さの疑い
- `confirmed_missing` - 本文欠落が確定

> 実装上の注意: obsflow の collect / summarize 更新では、人間が付与した `content_*` プロパティを消さずに保持する。
